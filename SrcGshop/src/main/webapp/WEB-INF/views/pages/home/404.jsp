<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!-- Page Header Start -->
<div class="container-fluid page-header wow fadeIn"
	data-wow-delay="0.1s">
	<div class="container">
		<h1 class="display-3 mb-3 animated slideInDown">404 Error</h1>
		<nav aria-label="breadcrumb animated slideInDown"></nav>
	</div>
</div>
<!-- Page Header End -->


<!-- 404 Start -->
<div class="container-xxl py-6 wow fadeInUp" data-wow-delay="0.1s">
	<div class="container text-center">
		<div class="row justify-content-center">
			<div class="col-lg-6">
				<i class="bi bi-exclamation-triangle display-1 text-primary"></i>
				<h1 class="display-1">404</h1>
				<h1 class="mb-4">Trang không tồn tại</h1>
				<p class="mb-4">Chúng tôi rất xin lỗi, trang mà bạn đang tìm
					kiếm không tồn tại của website này! Bạn có thể quay về trang chủ .</p>
				<a class="btn btn-primary rounded-pill py-3 px-5"
					href="/Gshop/trang-chu">Về trang chủ</a>
			</div>
		</div>
	</div>
</div>